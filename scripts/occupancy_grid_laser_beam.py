...
"""
    Creating map using laser beam inverse model, already described in prob robotics book, chapter about cartographing
"""
...
# Creating map much bigger than inspected location, because otherwise we will get index out of range errors in console
# because sensor will get pixel out of map range
FLOOR_SIZE_X = 5  # meters
FLOOR_SIZE_Y = 5  # meters
...
class Mapping:
    def __init__(self):
        ...
        # for scan msg -- CHANGE IT !!!
        self.from_frame = "camera_bottom_screw_frame" #"lidar_link"
        self.to_frame = "map"
    ...
    def likelihood_model(self, robot_pose, ranges):
        ...
        for k in range(len(ranges)):
            if ranges[k] > self.range_max or np.isnan(ranges[k]): continue
            dist = 1000
            #laser point
