...
if __name__ == '__main__':
	...
	try:
		...
		map_pub = rospy.Publisher('/best_map', OccupancyGrid, queue_size=10)
		...
		while not rospy.is_shutdown():
			...
			# Lidar measurements
			msgScan = rospy.wait_for_message('/scan_filtered', LaserScan)
			...
			# rospy.loginfo(f"\ndistances_x: {len(distances_x)} distances_y: {len(distances_y)}\n")
			...
			cv2.imshow("BGR map", gridMap.to_BGR_image())
			cv2.waitKey(2)
			...
			map_pub.publish(gridMap.to_occupancy_grid(x1, y1, theta_odom, seq_number))
			seq_number += 1
